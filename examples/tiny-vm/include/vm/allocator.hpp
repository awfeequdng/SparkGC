...
namespace vm {
    ...
    namespace memory_handler {
        ...
        static void construct(byte_t *ptr, ArgsT &&...args) {
            ::new(reinterpret_cast<T *>(ptr)) T(std::forward<ArgsT>(args)...);
        }

        template<typename T>
        static void destroy(byte_t *ptr) {
            reinterpret_cast<T *>(ptr)->~T();
        }

        template<typename T, typename...ArgsT>
        static T *alloc_new(allocator_base *alloc, ArgsT &&...args) {
        ...
        template<typename T>
        static void alloc_delete(allocator_base *alloc, T *ptr) {
            reinterpret_cast<T *>(ptr)->~T();
