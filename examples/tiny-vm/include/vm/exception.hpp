...
namespace vm {
    ...
    public:
        ...
        default_exception_handler(const std::initializer_list<exception_handler::listener_t> &lst) : m_listener_list(
            lst) {}
        ...
        virtual void raise(std::exception *e, int level, allocator_base *alloc = &default_alloc) override {
            ...
            if (level < 0)
                ...
                if (level > 20)
                    raise(memory_handler::alloc_new<std::range_error>(&default_alloc,
                        "CovVM: Wrong range of error level."), 1);
                else if (level > 10)
    ...
    void vm_throw(int level, ArgsT &&...args) {
